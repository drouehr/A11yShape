...
def getModificationPrompts(code, text, imgs):
    if len(code) > 0:
        content = [
            {"type": "text", "text": "Given the OpenScad code, modify the code to "+text+". Output only the modified OpenScad code and nothing else请注意：请使用中文返回结果"},
            {"type": "text", "text": code},
        ]
        for img in imgs:
            content.append({
        ...
        content = [
            {"type": "text", "text": "Generate the OpenScad code to "+text+". Output only the OpenScad code and nothing else请注意：请使用中文返回结果"},
        ]
