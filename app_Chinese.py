...
def getDescriptionPrompts(code, text, prevCode, fullCode, partCode, imgs, fullImgs, prevImgs):
    ...
    除了以上的部分，为了帮助盲人建立想象，请遵守以下原则：1.在描述模型的组成时候，必须描述模型部件之间的空间关系。 2.当你在涉及描述模型部件的大小角度等可以量化的特征时，你不能使用模糊的描述例如「更大一些」，而是使用具体的数值（如果有），或者使用相对的大小关系例如「<组件1>是<组件2>的2倍大」这种描述方式。 3. 如果你的描述将涉及到比喻，请确保这是盲人生活中可以接触到的物品或者了解的概念。比如说当你描述这是一个蜘蛛的话，大多数视力正常的人可能看到过就知道是什么形状，但是盲人只知道那些触摸过的物品的形状。 4.你的描述长度应该是盲人和屏幕阅读器感到舒适的阅读长度，不宜过度表达而影响阅读体验。
    最后，请注意你的描述长度应该是盲人和屏幕阅读器感到舒适的阅读长度。
    """
    if len(text) > 0:
        instructions = text
    ...
    if len(fullCode) > 0:
        ...
        content = [
                    ...
                    {"type": "text", "text": "除了以上的部分，为了帮助盲人建立想象，请遵守以下原则：1.在涉及到模型的可视特征例如外观时候，请注意使用空间关系来帮助盲人用户理解。 2.当涉及描述大小角度等可以量化的特征时，你不能使用模糊的描述例如「更大一些」，而是使用具体的数值（如果有），或者使用相对的大小关系例如「变大了两倍」这种描述方式。 3. 如果你的描述将涉及到比喻，请确保这是盲人生活中可以接触到的物品或者了解的概念，因为盲人只能类比触摸过的物品的形状。 4.最后，请注意你的描述长度应该是盲人和屏幕阅读器感到舒适的阅读长度，不宜过度表达而影响阅读体验。请使用中文表达结果."},
                ]
        for img in prevImgs:
            content.append({
                            "type": "image_url",
        ...
        content = [
                    {"type": "text", "text": text},
                    {"type": "text", "text": "和盲人对话时候，请遵守以下原则：1.在需要向盲人描述模型时候，可以描述相对的空间关系。 2.当你在涉及描述大小角度等可以量化的特征时，你不能使用模糊的描述例如「更大一些”」，而是使用具体的数值（如果有），或者使用相对的大小关系例如「<组件1>是<组件2>的2倍大」这种描述方式。 3. 如果你的描述将涉及到比喻，请确保这是盲人生活中可以接触到的物品或者了解的概念，盲人只知道那些触摸过的物品的形状。 4.最后，你的回复长度应该是盲人和屏幕阅读器感到舒适的阅读长度，不宜过度表达而影响盲人的阅读体验。"},
                ]
    else:
